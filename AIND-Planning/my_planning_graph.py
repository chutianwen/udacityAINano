...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        node_s_level = self.s_levels[level]
        hash_to_node = {hash(node): node for node in node_s_level}
        for action in self.all_actions:
            action_node = PgNode_a(action)
            precond_nodes = action_node.precond_s_nodes()
            # if precond_nodes are subset of state nodes at given level, if true then such action is legal.
            # compare set with set
            if precond_nodes < node_s_level:
                self.a_levels[level].add(action_node)
                for precond_node in precond_nodes:
                    node_parent = hash_to_node[hash(precond_node)]
                    node_parent.children.add(action_node)
                    action_node.parents.add(node_parent)
    ...
    def add_literal_level(self, level):
        ...
        # create a new level of state nodes, prepare for insertion of effect nodes from actions.
        self.s_levels.append(set())

        hash_to_node = dict()
        for node_action in self.a_levels[level-1]:
            for precond_node in node_action.effnodes:
                key = hash(precond_node)
                if key not in hash_to_node:
                    hash_to_node[key] = precond_node
                    self.s_levels[level].add(precond_node)

                hash_to_node[key].parents.add(node_action)
                node_action.children.add(hash_to_node[key])
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        flag1 = any(effect_rem in node_a2.action.effect_add for effect_rem in node_a1.action.effect_rem)
        flag2 = any(effect_rem in node_a1.action.effect_add for effect_rem in node_a2.action.effect_rem)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        flag1 = any(effect_rem in node_a2.action.precond_pos for effect_rem in node_a1.action.effect_rem)
        flag2 = any(effect_rem in node_a1.action.precond_pos for effect_rem in node_a2.action.effect_rem)
        return flag1 or flag2
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        flag = any(parent1.is_mutex(parent2) for parent1 in node_a1.parents for parent2 in node_a2.parents)
        return flag
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        node_s2.is_pos = not node_s2.is_pos
        if node_s2 == node_s1:
            node_s2.is_pos = not node_s2.is_pos
            return True
        else:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        parent_node1 = node_s1.parents
        parent_node2 = node_s2.parents
        return all(parent_1.is_mutex(parent_2) for parent_1 in parent_node1 for parent_2 in parent_node2)
    ...
    def h_levelsum(self) -> int:
        ...
        for lv in range(len(self.s_levels)):
            if len(goals) == 0:
                break

            # have to be goals[:] rather than goals, need to modify during iteration
            for goal in goals[:]:
                if PgNode_s(goal, True) in self.s_levels[lv]:
                    level_sum += lv
                    goals.remove(goal)
        return level_sum
